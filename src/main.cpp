...
#if defined(NDEBUG)
# error "Oryxcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc68381c9a1da34cc4bd2e979af50f6663b879c2b59ac9ad4a75c21d562f82e5a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Oryxcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Oryxcoin Signed Message:\n";
...
{
    // Oryxcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	// bigger rewards to first 4667 blocks
	if (nHeight <= 4667)
		return 15000 * COIN;
	
    // Subsidy is cut in half every 500000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 500000); // Oryxcoin: 500k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Oryxcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Oryxcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Oryxcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Oryxcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Paul walker died November 30, 2013, at about 3:30 p.m";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x6b6ec12a57d688e64cc1428aaa4ba81d6482212484a564616c37ca7fa228f66a"));
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb7, 0xdc }; // Oryxcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("OryxcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("OryxcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static OryxcoinMiner(CWallet *pwallet)
{
    printf("OryxcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("oryxcoin-miner");
    ...
    try { loop {
        ...
        printf("Running OryxcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("OryxcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&OryxcoinMiner, pwallet));
}
